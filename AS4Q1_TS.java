...
Now when we calculate each of the salary, it turns out to be:
...
public class AS4Q1_TS {
    public static void main(String[] args) {
        Scanner sc=new Scanner(System.in);
        System.out.println("Enter basic salary followed by grade");
        double a=sc.nextInt();
        String b=sc.nextLine();
        char abc=b.charAt(1);
        double inc=a+(a*0.5)+(a*0.2)-(a*0.11);
        switch (abc) {
            case 'A':
                System.out.println(Math.round(inc) + 1700);
                break;
            case 'B':
                System.out.println(Math.round(inc) + 1500);
                break;
            default:
                System.out.println(Math.round(inc) + 1300);
                break;
        }
    }
}
